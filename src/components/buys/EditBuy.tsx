...
type BuyFormEditProps = {
    buy: BuysFormDataInfo,
    dataAuth: AuthPermissions,
    onClose: () => void
}

export default function EditBuys({ buy, dataAuth, onClose }: BuyFormEditProps) {
    const navigate = useNavigate()
    const location = useLocation()
    const queryParams = new URLSearchParams(location.search)
    const buysId = queryParams.get("editBuy")!
    const [editBuy, setEditBuy] = useState(buy);
    ...
    const [dataProducts, setDataProducts] = useState<TempPurchasingFormDataDetails[]>(sessionStorage.getItem("tempProductsEditBuys") ? JSON.parse(sessionStorage.getItem("tempProductsEditBuys")!) : [])
    ...
    useEffect(() => {
        ...
        if (editBuy.metodo_pago[0].metodo == "Tarjeta") {
            setTarjeta(true);
            setMontoTarjeta(editBuy.metodo_pago[0].monto);
            setObservacionesTarjeta(editBuy.metodo_pago[0].observaciones!);
        }

        if (editBuy.metodo_pago[0].metodo == "Transferencia") {
            setTransferencia(true);
            setMontoTransferencia(editBuy.metodo_pago[0].monto);
            setObservacionesTransferencia(editBuy.metodo_pago[0].observaciones!);
        }

        if (editBuy.metodo_pago[0].metodo == "Cheque") {
            setCheque(true);
            setMontoCheque(editBuy.metodo_pago[0].monto);
            setObservacionesCheque(editBuy.metodo_pago[0].observaciones!);
        }
    }, [editBuy])
    ...
    function handleCalculateTotal() {
        ...
        editBuy.subtotal = sum.toFixed(2);
    ...
    function handleCalculateTotalWithTaxes() {
        ...
        editBuy.total = result.toFixed(2);
    ...
    const handleSelectionSupplier = (dataSupplier: SupplierFormDataInfo) => {
        ...
        setEditBuy({ ...editBuy, id_proveedor: dataSupplier.id })
    }
    ...
    const {
        handleSubmit,
        reset
    } = useForm<BuysFormDataInfo>({ defaultValues: editBuy });
    ...
    function onClickClearForm() {
        ...
        sessionStorage.removeItem("tempProductsEditBuys");
        setValorImpuesto(0);
        setTotal(0);
        setTaxesValue(15);
        setIsChecked(false);
    ...
    const onSubmitCreateBuys = () => {
        ...
        if (openAlertDialogDeleted == null) {
            ...
            editBuy.detalles_compra = dataStorage;
            editBuy.impuesto_manual = [{
            ...
            const formData = editBuy;
            const data = { buysId, formData };
            mutate(data);
        }
    }
    ...
    return (
        <DialogContent
            ...
            className="[&>button]:hidden h-[98%] w-full md:max-w-[97%] px-2 py-4 md:p-6 scrollbar-thin-custom touch-pan-x touch-pan-y scroll-smooth overflow-scroll"
        >
            <DialogHeader>
                    ...
                    <fieldset className="flex w-full h-72 flex-col gap-y-6 items-center border border-gray-300 dark:border-gray-600 rounded-lg p-4">
                        ...
                        <div className="w-full">
                            ...
                            <input
                                ...
                                value={editBuy.numero_factura_proveedor}
                                onChange={(e) => setEditBuy({ ...editBuy, numero_factura_proveedor: e.target.value })}
                                type="text"
                                required
                                minLength={2}
                        ...
                        <div className="flex flex-1 w-full items-start justify-center">
                            <div className="w-full">
                                ...
                                <textarea
                                    ...
                                    className="w-full border border-gray-300 dark:border-gray-600 outline-none rounded-md py-1 px-2"
                                    placeholder="Ejemplo: xxxxx..."
                                    value={editBuy.observaciones}
                                    onChange={(e) => setEditBuy({ ...editBuy, observaciones: e.target.value })}
                                    rows={6}
                                />
                            </div>
                        </div>
