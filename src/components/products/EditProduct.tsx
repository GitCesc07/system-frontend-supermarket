...
import { ChevronDown, ChevronUp, ImageIcon, ImageOff, ImageUp, Save, Search, Upload, X } from "lucide-react";
import type { ErrorData } from "@/types/errors.interface";
import { toast } from "sonner";
import { stateValue } from "@/types/state.interface";
import type { ProductFormDataAdd, ProductFormDataInfo } from "@/types/products.interface";
import { updateProduct, uploadImage } from "@/apis/products.apis";
import ErrorMessage from "../error-message";
import { getBrands } from "@/apis/brand.apis";
import { getAllCategories } from "@/apis/categories.apis";
import { formatDateInput } from "@/utils/utilidad";

export default function EditProduct({ product, onClose }: { product: ProductFormDataInfo, onClose: () => void }) {
    ...
    useEffect(() => {
        dataBrands?.forEach(brand => {
            if (brand.id == editProduct.id_marca) {
    ...
    }, [dataBrands, editProduct.id_marca]);
    ...
    useEffect(() => {
        dataCategories?.forEach(category => {
            if (category.id == editProduct.id_categoria) {
    ...
    }, [dataCategories, editProduct.id_categoria]);
    ...
    const {
        handleSubmit,
        reset
    } = useForm<ProductFormDataAdd>({ defaultValues: editProduct });
    ...
    const { mutate: mutateUpload } = useMutation({
        ...
        onSuccess: (data) => {
            ...
            editProduct.imagen_url = data.result.publicUrl;
            setImageUpload(true);
        }
    })
    ...
    const onsubmitCreateProduct = () => {
        ...
        if (editProduct.id_marca == "" || editProduct.id_marca == null || editProduct.id_marca == undefined) {
        ...
        if (editProduct.id_categoria == "" || editProduct.id_categoria == null || editProduct.id_categoria == undefined) {
    ...
    return (
        <DialogContent
            ...
            className="[&>button]:hidden w-full md:max-w-xl h-[95%] md:max-h-[95%] scrollbar-thin-custom scroll-smooth mx-auto touch-pan-y overflow-scroll"
        >
            <DialogHeader>
                    ...
                    <div className="flex items-start md:items-center md:flex-row flex-col gap-y-4 md:gap-x-14 md:justify-center">
                        ...
                        <div className="md:w-full flex gap-x-3 items-center flex-row-reverse">
                            ...
                            <input
                                ...
                                id="expiracion"
                                type='checkbox'
                                checked={editProduct.expiracion == 1 ? true : false}
                                onChange={(e) => {

                                    if (editProduct.expiracion == 1 || e.target.checked == true) {
...

                                                {
                                                    filteredBrands?.map((brand, index) => (
                                                        <li
                                                            ...
                                                            onClick={() => {
                                                                setEditProduct({ ...editProduct, id_marca: brand.id });
                                                                setSearchTermBrand("");
                                                                setOpenComboBoxBrand(!openComboBoxBrand);
...

                                                {
                                                    filteredCategories?.map((category, index) => (
                                                        <li
                                                            ...
                                                            className="hover:bg-gray-300 py-1 px-4 cursor-pointer"
                                                            onClick={() => {
                                                                setEditProduct({ ...editProduct, id_categoria: category.id });
