...
import { ChevronDown, ChevronUp, Edit, MessageCircleQuestion, PlusIcon, Save, Search, Trash2 } from "lucide-react";
import type { ErrorData } from "@/types/errors.interface";
import { toast } from "sonner";
import { Button } from "../ui/button";
import { getExpiredProductsById, updateExpiredProducts } from "@/apis/expiredProducts.apis";
...
type ExpiredproductsFormEditProps = {
    expiredProducts: ExpiredProductsFormDataInfo,
    onClose: () => void
}

export default function EditExpiredProducts({ expiredProducts, onClose }: ExpiredproductsFormEditProps) {
    const navigate = useNavigate()
    const location = useLocation()
    const queryParams = new URLSearchParams(location.search)
    const expiredProductsId = queryParams.get("editExpiredProducts")!
    const [editExpiredProducts, setEditExpiredProducts] = useState(expiredProducts);

    const [searchTerm, setSearchTerm] = useState("");
    const [openAlertDialogDeleted, setOpenAlertDialogDeleted] = useState<TempPurchasingFormData | null>(null);
    const [dataProducts, setDataProducts] = useState<TempPurchasingFormDataDetails[]>(sessionStorage.getItem("tempProductsEditExpiredProducts") ? JSON.parse(sessionStorage.getItem("tempProductsEditExpiredProducts")!) : [])
    ...
    const { data } = useQuery({
        queryKey: ["expiredProducts", expiredProductsId],
        queryFn: () => getExpiredProductsById({ id: expiredProductsId }),
        enabled: !!expiredProductsId,
        retry: false
    });
    ...
    useEffect(() => {
        sessionStorage.setItem("tempProductsEditExpiredProducts", JSON.stringify(dataProducts));
    }, [dataProducts]);
    ...
    const { mutate } = useMutation({
        mutationFn: updateExpiredProducts,
    ...
    const {
        handleSubmit,
        reset
    } = useForm<ExpiredProductsFormDataInfo>({ defaultValues: editExpiredProducts });
    ...
    const onSubmitCreateExpiredProducts = () => {
        const formData = editExpiredProducts;
        const data = { expiredProductsId, formData };
        mutate(data);
    }
    return (
        <DialogContent
            ...
            >
                <div className="w-full">
                    ...
                    <input
                        ...
                        id="observaciones"
                        value={editExpiredProducts.observaciones}
                        onChange={(e) => {
                            setEditExpiredProducts({ ...editExpiredProducts, observaciones: e.target.value });
                        }}
                        minLength={3}
                        maxLength={50}
                        required
